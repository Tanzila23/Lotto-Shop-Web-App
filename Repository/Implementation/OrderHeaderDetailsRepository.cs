...
{
    public class OrderHeaderDetailsRepository : IOrderHeaderDetailsRepository
    {
        public List<OrderHeaderVm> GetOrderHeaderDetails()
        {
            ...
            {
                sqlConn.Open();
                List<OrderHeaderVm> dt = new List<OrderHeaderVm>();
                using (SqlCommand sqlComm = new SqlCommand("USP_LIST_ORDER_SUMMARY", sqlConn))
                {

                    sqlComm.CommandType = System.Data.CommandType.StoredProcedure;
                    sqlComm.Parameters.AddWithValue("@OrderNo", 0);
                    ...
                    using (SqlDataReader sdr = sqlComm.ExecuteReader())
                    {
                        while (sdr.Read())
                        {
                            try
                            {
                                OrderHeaderVm dtc = new OrderHeaderVm();
